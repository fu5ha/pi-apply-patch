...
import { PATCH_PREVIEW_MAX_CHARS, PATCH_PREVIEW_MAX_LINES } from "../src/patch/constants.js";
...
*** End Patch`,
    ...
    it("#given large preview #when rendering result expanded #then shows all lines", () => {
        ...
        const component = tool.renderResult?.(
            ...
            {
                cwd: "/workspace/project",
                toolCallId: "result-large-expanded",
                args: { input: "" },
            } as never,
        );
        const rendered = component?.render(400).join("\n") ?? "";
